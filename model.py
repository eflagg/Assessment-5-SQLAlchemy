...
class Model(db.Model):
    ...
    brand_name = db.Column(db.String, db.ForeignKey('brands.name'))
    name = db.Column(db.String(50), nullable=False)

    brand = db.relationship('Brand', backref=db.backref('models'))

    def __repr__(self):
        return "<Model id=%s year=%s brand=%s name=%s>" % (self.id, self.year, self.brand_name, self.name)
...
class Brand(db.Model):
    ...
    discontinued = db.Column(db.String(50))
